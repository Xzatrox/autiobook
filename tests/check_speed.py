...
def benchmark_disk_io(size_mb=256):
    ...
    with tempfile.NamedTemporaryFile(delete=True) as f:
        ...
        print(f"Write: {size_mb / write_t:.0f} MB/s | Read: {size_mb / read_t:.0f} MB/s")
...
def benchmark_attention(device_type, dtype, max_mem):
    ...
    configs = ([("Flash", [SDPBackend.FLASH_ATTENTION]), ("MemEff", [SDPBackend.EFFICIENT_ATTENTION])]
               if "cuda" in device_type else [("Default", None)])
    cleanup()
    seq_len = 2048
    elem_size = 2 if dtype != torch.float32 else 4
    for name, bk in configs:
        ...
        for i in range(MAX_BATCH_DOUBLINGS):
            ...
            try:
                ...
                with ctx:
                    avg_t = timed_run(lambda: F.scaled_dot_product_attention(q, k, v), device_type)
...
def benchmark_conv1d(device_type, dtype, max_mem):
    ...
    elem_size = 2 if dtype != torch.float32 else 4
    conv = torch.nn.Conv1d(channels, channels, kernel, padding=3).to(device, dtype=dtype)
    for i in range(MAX_BATCH_DOUBLINGS):
        batch_size = 1 << i
...
def benchmark_conv_transpose1d(device_type, dtype, max_mem):
    ...
    for in_ch, out_ch, kernel, stride in configs:
        try:
            ...
            conv_t = torch.nn.ConvTranspose1d(in_ch, out_ch, kernel, stride).to(device, dtype=dtype)
            out = conv_t(x)
            avg_t = timed_run(lambda: conv_t(x), device_type)
            print(f"{in_ch}->{out_ch} k{kernel}s{stride:<6} {avg_t*1000:>7.2f}ms {out.shape[-1]:>8}")
            length = out.shape[-1]
            del x, conv_t, out
...
def benchmark_audio_transforms(device_type, dtype, max_mem):
    ...
    for duration in [1, 5, 10]:
        try:
            ...
            avg_t = timed_run(lambda: torch.stft(x.float(), n_fft, window=window, return_complex=True), device_type)
            rtf = duration / avg_t
...
def benchmark_mel_spectrogram(device_type, dtype, max_mem):
    ...
    for i in range(n_mels):
        if bins[i+1] > bins[i]:
            mel_basis[i, bins[i]:bins[i+1]] = torch.linspace(0, 1, bins[i+1] - bins[i])
        if bins[i+2] > bins[i+1]:
            mel_basis[i, bins[i+1]:bins[i+2]] = torch.linspace(1, 0, bins[i+2] - bins[i+1])
    window = torch.hann_window(n_fft, device=device)
    for duration in [1, 5, 10]:
        try:
            ...
            print(f"{duration}s{'':<7} {avg_t*1000:>7.1f}ms {mel.shape[-1]:>8} {duration/avg_t:>7.0f}x")
            del audio, mel
        except:
            break
...
def benchmark_kv_cache(device_type, dtype, max_mem):
    ...
    for seq_len in [256, 1024, 4096]:
        ...
        if cache_bytes > max_mem:
            break
        try:
            kv = torch.randn(NUM_LAYERS, 2, 1, NUM_HEADS, seq_len, HEAD_DIM, device=device, dtype=dtype)
            new_kv = torch.randn(NUM_LAYERS, 2, 1, NUM_HEADS, 1, HEAD_DIM, device=device, dtype=dtype)
            ...
            write_t = timed_run(lambda: kv[:, :, :, :, -1:, :].copy_(new_kv), device_type)
            write_bw = new_kv.numel() * elem_size / write_t / 1e9
            print(f"{seq_len:<8} {format_bytes(cache_bytes):>10} {read_bw:>8.0f}GB/s {write_bw:>8.1f}GB/s")
            del kv, new_kv
        except:
            break
...
def benchmark_autoregressive(device_type, dtype, max_mem):
    ...
    for cache_len in [256, 1024, 2048]:
        ...
        if cache_bytes * 2 > max_mem:
            break
        try:
            q_proj = torch.nn.Linear(HIDDEN_SIZE, HIDDEN_SIZE, bias=False).to(device, dtype=dtype)
            k_proj = torch.nn.Linear(HIDDEN_SIZE, HIDDEN_SIZE, bias=False).to(device, dtype=dtype)
            v_proj = torch.nn.Linear(HIDDEN_SIZE, HIDDEN_SIZE, bias=False).to(device, dtype=dtype)
            o_proj = torch.nn.Linear(HIDDEN_SIZE, HIDDEN_SIZE, bias=False).to(device, dtype=dtype)
            hidden = torch.randn(1, 1, HIDDEN_SIZE, device=device, dtype=dtype)
            k_cache = torch.randn(1, NUM_HEADS, cache_len, HEAD_DIM, device=device, dtype=dtype)
            v_cache = torch.randn(1, NUM_HEADS, cache_len, HEAD_DIM, device=device, dtype=dtype)
...
def benchmark_tts_batch_scaling(device_type, dtype, max_mem):
    ...
    print(f"{'Batch':<8} {'Prefill':>10} {'Decode':>10} {'Total':>10} {'Throughput':>12} {'VRAM':>10}")
    print(f"{'-'*64}")
    cleanup()
    ...
    for batch_size in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]:
        # check memory
        mem_needed = batch_size * NUM_LAYERS * 2 * NUM_HEADS * (seq_len + decode_steps) * HEAD_DIM * elem_size
        ...
        try:
            ...
            k_cache = torch.zeros(batch_size, NUM_HEADS, seq_len + decode_steps, HEAD_DIM, device=device, dtype=dtype)
            v_cache = torch.zeros(batch_size, NUM_HEADS, seq_len + decode_steps, HEAD_DIM, device=device, dtype=dtype)

            def prefill():
                q = q_proj(hidden).view(batch_size, seq_len, NUM_HEADS, HEAD_DIM).transpose(1, 2)
                k = k_proj(hidden).view(batch_size, seq_len, NUM_HEADS, HEAD_DIM).transpose(1, 2)
                v = v_proj(hidden).view(batch_size, seq_len, NUM_HEADS, HEAD_DIM).transpose(1, 2)
                k_cache[:, :, :seq_len, :] = k
                v_cache[:, :, :seq_len, :] = v
            ...
            hidden_dec = torch.randn(batch_size, 1, HIDDEN_SIZE, device=device, dtype=dtype)
            cache_len = seq_len + 10

            def decode_step():
                q = q_proj(hidden_dec).view(batch_size, 1, NUM_HEADS, HEAD_DIM).transpose(1, 2)
                k_new = k_proj(hidden_dec).view(batch_size, 1, NUM_HEADS, HEAD_DIM).transpose(1, 2)
                v_new = v_proj(hidden_dec).view(batch_size, 1, NUM_HEADS, HEAD_DIM).transpose(1, 2)
                k_cache[:, :, cache_len:cache_len+1, :] = k_new
                v_cache[:, :, cache_len:cache_len+1, :] = v_new
                return F.scaled_dot_product_attention(q, k_cache[:, :, :cache_len+1, :], v_cache[:, :, :cache_len+1, :])
            ...
            print(f"{batch_size:<8} {prefill_t*1000:>8.1f}ms {total_decode_t*1000:>8.0f}ms {total_t*1000:>8.0f}ms {throughput:>10.1f}/s {vram_used:>10}")
...
def benchmark_qwen_tts_real(device_type, dtype, max_mem):
    ...
    if device_type == "cpu":
        ...
        return
    print(f"\n{'='*60}")
    print(f"Benchmark: Real Qwen-TTS Batch Processing")
    print(f"{'='*60}")
    ...
    for batch_size in [1, 2, 4, 8, 16, 32, 64]:
        try:
            ...
            print(f"{batch_size:<8} {elapsed:>8.2f}s {audio_duration:>8.1f}s {vram_str:>10} {rtf:>7.1f}x  ({per_sample:.1f}s/sample)")
...
def benchmark_model_load(device_type, dtype, max_mem):
    print(f"\n{'='*60}")
    print(f"Benchmark: Model Loading (simulated 1.7B params)")
    print(f"{'='*60}")
    ...
    with tempfile.NamedTemporaryFile(delete=True, suffix='.bin') as f:
        ...
        print(f"mmap:   {test_mb / mmap_t:.0f} MB/s (full model ~{mmap_t * scale:.1f}s)")
        # direct read
        f.seek(0)
        os.posix_fadvise(f.fileno(), 0, len(data), os.POSIX_FADV_DONTNEED)
        start = time.time()
        ...
        print(f"direct: {test_mb / direct_t:.0f} MB/s (full model ~{direct_t * scale:.1f}s)")
        # H2D
        if device_type != "cpu" and torch.cuda.is_available():
            ...
            print(f"H2D:    {test_mb / h2d_t:.0f} MB/s (full model ~{h2d_t * scale:.1f}s)")
...
def main():
    ...
    selected = set(benchmarks.keys()) if "all" in args.benchmarks else {
        b for b in args.benchmarks if b in benchmarks
    }
